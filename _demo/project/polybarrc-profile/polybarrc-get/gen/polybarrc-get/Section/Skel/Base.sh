...
is_debug () {
	if [ "$DEBUG_POLYBARRC_GET" = "true" ]; then
		return 0
	fi
...
base_var_init () {
	...
	if [ "none${THE_CMD_FILE_NAME}" = 'none' ]; then
		THE_CMD_FILE_NAME='polybarrc-get'
	fi

	THE_CMD_FILE_PATH="${THE_CMD_DIR_PATH}/$THE_CMD_FILE_NAME"

	## https://github.com/samwhelp/note-about-polybar/blob/gh-pages/_demo/project/polybarrc-profile/polybarrc-get/polybarrc-get
	THE_SELF_UPDATE_URL='https://raw.githubusercontent.com/samwhelp/note-about-polybar/gh-pages/_demo/project/polybarrc-profile/polybarrc-get/polybarrc-get'
	...
	## polybar
	THE_POLYBARRC_DIR_NAME="polybar"
	THE_POLYBARRC_DIR_PATH="$HOME/.config/$THE_POLYBARRC_DIR_NAME"

	THE_POLYBARRC_FILE_NAME="polybar.conf"
	THE_POLYBARRC_FILE_PATH="$THE_POLYBARRC_DIR_PATH/$THE_POLYBARRC_FILE_NAME"

	## polybarrc-profile
	THE_POLYBARRC_PROFILE_DIR_NAME="polybarrc-profile"
	THE_POLYBARRC_PROFILE_DIR_PATH="$HOME/.local/share/$THE_POLYBARRC_PROFILE_DIR_NAME"
...
base_var_dump () {
	...
	util_debug_echo "THE_POLYBARRC_DIR_NAME=$THE_POLYBARRC_DIR_NAME"
	util_debug_echo "THE_POLYBARRC_DIR_PATH=$THE_POLYBARRC_DIR_PATH"

	util_debug_echo "THE_POLYBARRC_FILE_NAME=$THE_POLYBARRC_FILE_NAME"
	util_debug_echo "THE_POLYBARRC_FILE_PATH=$THE_POLYBARRC_FILE_PATH"

	util_debug_echo "THE_POLYBARRC_PROFILE_DIR_NAME=$THE_POLYBARRC_PROFILE_DIR_NAME"
	util_debug_echo "THE_POLYBARRC_PROFILE_DIR_PATH=$THE_POLYBARRC_PROFILE_DIR_PATH"
