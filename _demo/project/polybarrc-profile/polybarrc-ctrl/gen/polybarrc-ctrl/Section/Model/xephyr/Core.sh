...
mod_xephyr () {
	...
	mod_xephyr_run_polybar "$1" "$2"

}


mod_xephyr_run_polybar () {
	...
	if [ "none$config_file_path" = "none" ]; then
		util_error_echo "DISPLAY=:100 polybar"
		DISPLAY=:100 polybar
	else
		util_error_echo "DISPLAY=:100 polybar -c $config_file_path"
		DISPLAY=:100 polybar -c $config_file_path
	fi
